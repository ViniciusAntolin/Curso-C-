...
/* Contando o numerod e ocorrencias */
select count(*) from funcionarios;
...
select count(*) as "Quantidade", sexo from funcionarios
group by sexo;

/* Mostrando o numero de funcionarios em cada departamento */
select count(*) as "QuantidadeFuncionario", departamento from funcionarios
...
/* Exibindo o minimo e maximo de salarios */
select min(salario) as "Menor Salario", max(salario) as "Maior Salario" from funcionarios

/* Exibindo o maixmo e o minímo de cada departamento */
select min(salario) as "Menor Salario", max(salario) as "Maior Salario", departamento 
from funcionarios
group by departamento

/* Exibindo o maximo e o minímo por sexo */
select min(salario) as "Menor Salario", max(salario) as "Maior Salario", sexo 
from funcionarios
group by sexo
...
/* Qual o montadente total que cada departamente recebe de salario */
select sum(salario) as "Total por Departamento",departamento from funcionarios
group by departamento

/* Por departamento, qual o total e a media paga para os funcionarios */
select departamento, avg(salario) as "Media salarial", sum(salario) as "Total por Departamento" from funcionarios
group by departamento
...
Consomem muito processamento em função de Join. Queries lentas. */

/* Data Science e B.I -> Focam em agregações e performance.
Não se preocupam com espaço em disco. Em B.I modelagem minima 
...
/* IMPORTANDO ARQUIVOS CSC 1 */
COPY MAQUINAS
FROM 'C:\temp\LogMaquinas.csv'
DELIMITER ','
CSV HEADER;

/* IMPORTANDO ARQUIVOS CSC 2 */
COPY MAQUINAS
FROM 'C:\temp\LogMaquinas.csv'
WITH (FORMAT CSV, 
...
/* QUAL A MODA DAS QUANTIDADE DE CADA MAQUINA */
SELECT MAQUINA, QTD, COUNT(*) AS MODA
FROM MAQUINAS
...
STDDEV_POP(COLUNA) -- DESVIO PADRÃO | STANDARD DESVIATION
VAR_POP(COLUNA) -- VARIÂNCIA
...
/* MEDIANA */
--MOD VERIFICA O RESTO DA DIVISÃO SE FOR 0 É PAR SE 1 IMPAR.
--CEIL É UMA FUNÇÃO DE ARREDONDAMENTO
...
/* RELATORIO COMPLETO DAS MEDIDAS ESTATISTICAS */
SELECT MAQUINA,
COUNT(*) AS QUANTIDADE,
SUM(QTD) AS SOMA,
ROUND(AVG(QTD),2) AS MEDIA,
...
SELECT MAQUINA,
MODE() WITHIN GROUP (ORDER BY QTD) 
FROM MAQUINAS
GROUP BY MAQUINA
ORDER BY 1
...
SELECT F.NOME AS NOMEFILME, G.NOME AS NOMEGENERO, L.DATA, L.DIAS, L.MIDIA
FROM GENERO G
INNER JOIN FILME F ON G.idgenero = F.id_genero
INNER JOIN LOCACAO L ON L.id_filme = F.idfilme
...
/* SELECT PARA TRAZER OS REGISTRO NOVOS */
...
INNER JOIN LOCACAO L
ON L.ID_FILME = F.IDFILME
WHERE L.idlocacao NOT IN (SELECT idlocacao FROM relatorio_locadora);
...
/* DROPANDO A TRIGGER PARA CHAMAR A FUNÇÃO */
DROP TRIGGER TG_RELATORIO ON LOCACAO;

/* CRIANDO A TRIGGER PARA CHAMAR A FUNÇÃO */
...
/* VAMOS DEIXAR ESSE PROCEDIMENTO AUTOMATICO POR MEIO DE UMA TRIGGER */
/* PRIMEIRO É CRIADO A FUNCTION */
CREATE OR REPLACE FUNCTION DELETE_REL()
RETURNS TRIGGER AS $$
BEGIN
...
/* CRIANDO A TRIGGER PARA CHAMAR A FUNÇÃO */
CREATE TRIGGER TG_RELATORIO_DEL
BEFORE DELETE ON LOCACAO
FOR EACH ROW
	EXECUTE PROCEDURE DELETE_REL();

select * from relatorio_locadora

select idlocacao from locacao

delete from locacao where idlocacao = 33

/* VÁRIASVEIS DUMMY PARA MACHINE LEARNING */
SELECT NOME, CARGO,
CASE WHEN SEXO = 'Feminino' THEN 1 ELSE 0 END AS SEXO
FROM FUNCIONARIOS

/* UTLIZANDO VALOR BOOLEANOS */
SELECT NOME, CARGO, (SEXO= 'Masculino') as sexo,
(SEXO= 'Feminino') as sexo
FROM FUNCIONARIOS

/* MESCLANDO AS TECNICAS -VARIAVEL DUMMY */
SELECT NOME, CARGO, 
CASE WHEN (SEXO= 'Masculino') = true THEN 1 ELSE 0 END,
CASE WHEN (SEXO= 'Feminino') = true THEN 1 ELSE 0 END
FROM FUNCIONARIOS

/* EXERCICIOS */

/* QUAL A MODA DOS SALARIOS. A MODA DOS SALÁRIOS TE DIZ ALGO DE RELEVANTE? */
SELECT --NOME, 
MODE() WITHIN GROUP (ORDER BY SALARIO) AS "MODA"
FROM FUNCIONARIOS

/* QUAL A MODA DEPARTAMENTAL = QUAL O DEPARTAMENTO QUE MAIS EMPREA */
SELECT DEPARTAMENTO, 
MODE() WITHIN GROUP (ORDER BY SALARIO) AS "MODA"
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO
...
/* QUAL DEPARTAMENDO DÁ UMA MAIOR PROBILIDADE DE GANHAR MAIS? --OUTDOOR */
SELECT DEPARTAMENTO,
COUNT(*) AS QUANTIDADE,
SUM(SALARIO) AS SOMA,
ROUND(AVG(SALARIO),2) AS MEDIA,
...
/* FILTRO BASEADOS EM STRINGS */
SELECT NOME, DEPARTAMENTO, SALARIO
FROM FUNCIONARIOS
WHERE DEPARTAMENTO = 'Ferramentas'; --case sensitive no filtro
...
/* FILTRO BASEADO EM UNICO TIPO E COLUNA - ATENÇÃO PARA A REGRA AND E OR
- EM RELACIONAMENTOS 1X1 O FILTRO AND TRATANDO DE UMA UNICA COLUNA SEMPRE DARÁ FALSO
- JÁ EM RELACIONAMENTOS 1XN O FILTRO AND PODE SER TRUE */
SELECT NOME, DEPARTAMENTO, SALARIO
FROM FUNCIONARIOS
WHERE DEPARTAMENTO = 'Ferramentas' AND DEPARTAMENTO = 'Lar';

/* FILTROS BASEADOS EM PADRÃO EM CARACTERES */
SELECT DEPARTAMENTO, SUM(SALARIO) AS SALARIO
FROM FUNCIONARIOS
WHERE DEPARTAMENTO LIKE 'Be%'
GROUP BY DEPARTAMENTO;

/* UTILIZANDO O CARACTER CORINGA NO MEEIO DA PALAVRA */
SELECT DEPARTAMENTO, SUM(SALARIO) AS SALARIO
FROM FUNCIONARIOS
...
HAVING É O WHERE DAS CLAUSULAS AGREGADOS, 
AS CLAUSULAS AGREGADAS EU TRABALHO NO HAVING 
E AS NÃO AGREGADAS NO WHERE 
HAVING É TENDO */
SELECT DEPARTAMENTO, SUM(SALARIO) AS SALARIO
FROM FUNCIONARIOS
WHERE DEPARTAMENTO LIKE 'Be%'
...
SELECT COUNT(*) AS QUANTIDADETOTAL, SEXO
FROM FUNCIONARIOS
...
COUNT(*) FILTER(WHERE SEXO = 'Masculino') as masculino,
COUNT(*) FILTER(WHERE DEPARTAMENTO = 'Books') as Books,
COUNT(*) FILTER(WHERE SALARIO > 140000) as Salario
FROM FUNCIONARIOS;
...
/* DESAFIO - CRIAR UMA COLUNA AO LADO DA COLUNA CARGO QUE DIA SE A PESSOA É ASSISTENTE OU NÃO */
SELECT NOME, CARGO,
(CARGO LIKE '%Assistant%') as "É ASSISTENTE?"
FROM FUNCIONARIOS